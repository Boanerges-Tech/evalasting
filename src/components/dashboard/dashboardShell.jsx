...
function SidebarItem({ label, active = false }) {
  return (
    <button
      className={[
        "flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-[12px] font-semibold transition",
        active
...
function ProductCard({ item }) {
  return (
    ...
    >
      ...
      <div className="p-4">
        <div className="text-[10px] font-semibold text-muted">{item.category}</div>
        <div className="mt-1 text-[13px] font-extrabold text-ink">{item.name}</div>
...
export default function DashboardShell() {
  ...
  return (
    <div className="mx-auto grid max-w-[1150px] gap-0 lg:grid-cols-[260px_1fr]">
      {/* Sidebar */}
      <aside className="rounded-l-3xl bg-[#024f4a] px-5 py-6 text-white">
      ...
      {/* Main */}
      <section className="rounded-r-3xl bg-[#f7f2ea] px-5 py-6 sm:px-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="text-[12px] font-semibold text-muted">Overview</div>
          </div>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          <MetricCard value={stats?.reservations ?? 0} label="Reservations" accent="text-brand-700" />
          <MetricCard value={stats?.orders ?? 0} label="My Orders" accent="text-[#4c8b63]" />
          <MetricCard value={stats?.addresses ?? 0} label="Addresses" accent="text-[#5673d6]" />
        </div>

        <div className="mt-6">
          <div className="text-[13px] font-extrabold text-ink">Quick Actions</div>
        ...
        <div className="mt-7">
          <div className="flex items-center justify-between">
            ...
            <Link to="/menu" className="text-[11px] font-semibold text-brand-700">
              See all
            </Link>
          </div>

          {loading ? (
            <div className="mt-4 text-[12px] text-muted">Loading recommendations...</div>
          ) : (
