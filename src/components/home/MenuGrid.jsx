  ...
  return (
    <div className="group block">
      <Link
        to={`/product/${item.id}`}
        state={{ product: item }}
        className="relative overflow-hidden rounded-3xl bg-white shadow-soft2 ring-1 ring-line block"
      >
      ...
      <div className="mt-3 flex justify-between items-center">
        <div className="text-[13px] font-extrabold text-ink">{item.name}</div>
        <button
          onClick={() => addToCart(item)}
          className="h-8 w-8 rounded-full bg-brand-600 text-white grid place-items-center hover:bg-brand-700 transition"
        >
          +
        </button>
      </div>
